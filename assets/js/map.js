...
// A region star badge: stars AND trophies, in exactly the grammar the top
// bar's fox chip already taught — "⭐ N 🏆 M" — so the map says how far a
// place has come without a word of text. (It briefly showed the latest
...
function renderBadge(badgeEl, iconName, count, tier, cups) {
  badgeEl.innerHTML = iconSVG(iconName, { x: -26, y: 0, size: 12 })
    ...
    + `<text class="region-stars" x="22" y="0" text-anchor="start">${cups}</text>`;
